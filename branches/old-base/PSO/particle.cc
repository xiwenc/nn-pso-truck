...
Particle::Particle(int d) {
	if (debugging) cout << "creating particle" << endl;
	_init(d);
}//constructor
...
Particle::~Particle(void) {
	if (debugging) cout << "destroying particle" << endl;
}//destructor
...
void Particle::update(Vec lbest) {
	if (debugging) cout << "updating particle" << endl;
...
bool Particle::processEval(float value) {
	if (debugging) cout << "updating personal best" << endl;
...
void Particle::_init(int d) {
	if (debugging) cout << "initializing particle" << endl;
	vector<float> v, l;
	for (int i = 0; i < d; ++i) {

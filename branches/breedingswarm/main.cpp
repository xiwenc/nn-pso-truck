// $Id: main.cpp 74 2010-12-21 18:23:55Z xcheng $
...
void SimulationLoop()
{
    ...
    ((Truck*) test)->getEnvironment(env);
    
    // get the simulation parameters
    Neural neural(config[CONFIG_IN_NODES], config[CONFIG_OUT_NODES],
            config[CONFIG_HIDDEN_LAYERS], config[CONFIG_NODES_PER_LAYER]);
    neural.setWeights(weights);
    neural.perform(env, params);

    ((Truck*) test)->setParameters(params);
...
int main(int argc, char** argv)
{
    ...
    config[CONFIG_SURVIVORS] = NPARTICLES/2;
    ...
    while ((c = getopt(argc, argv, "d:i:o:l:n:t:p:a:x:")) != -1) {
        switch (c) {
            ...
            case 'p':
                ...
                config[CONFIG_SURVIVORS] = atoi(optarg)/2;
                break;
            case 'a':
    ...
    // initialize a BS
    swarm = new BS(config[CONFIG_NUM_PARTICLES], config[CONFIG_DIMENSIONS],
            test);
    ...
    for (int i = 0; i < config[CONFIG_ITERATIONS]; i++) {
        ...
        swarm->update(inertia2, mrate2, test);
    ...
    // dit moet in een loop, maar waar?!?
    //Background* b;
    //b = new Background(weights, true);
    //b->Fitness(test);
    //delete b;
    delete test;
    test = Truck::Create();

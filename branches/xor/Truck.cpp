...
void Truck::getEnvironment(double* env) {
    ...
    env[0] = m_vehicle->GetPosition().x - o_bodyOffset.x;
    env[1] = m_vehicle->GetPosition().x;
    env[2] = m_vehicle->GetPosition().y;
    env[3] = m_vehicle->GetLinearVelocity().Length();
    env[4] = m_vehicle->GetAngle() / (2 * M_PI);
    env[5] = m_vehicle->GetInertia();
    env[6] = m_leftAxle->GetAngularVelocity();
    env[7] = m_rightAxle->GetAngularVelocity();
    env[8] = m_leftJoint->GetMotorSpeed();
    env[9] = m_rightJoint->GetMotorSpeed();
}
...
void Truck::setParameters(double* params) {
    ...
    m_leftJoint->SetMotorSpeed(params[0]);
    m_rightJoint->SetMotorSpeed(params[1]);
}

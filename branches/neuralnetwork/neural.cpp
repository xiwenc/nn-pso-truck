// $Id: neural.cpp 36 2010-11-18 14:29:03Z xcheng $
#include "neural.h"
...
void Neural::perform(double* inputValues, double* outputValues) {
    ...
    double aH[_hiddenLayers][_nodesPerLayer];
    ...
    for (int h = 0; h < _hiddenLayers; h++) {
           for (int y = 0; y < _nodesPerLayer; y++) {
               ...
               // hidden to hidden
               for (int x = 0; x < _hiddenLayers * _nodesPerLayer; x++) {
                   for (int n = 0; n < _hiddenLayers * _nodesPerLayer; n++) {
                       dSum += aH[h][y] * _weightsHidden[x][y];
                   }
               }
               aH[h][y] = sigmoid(dSum);
           }
    ...
    for (int i = 0; i < _outputs; i++) {
        ...
        for (int h = 0; h < _hiddenLayers; h++) {
            for (int y = 0; y < _nodesPerLayer; y++) {
                dSum += aH[h][y] * _weightsOutputs[i][h * _nodesPerLayer + y];
            }
        }
